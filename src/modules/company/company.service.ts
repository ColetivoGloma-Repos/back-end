...
export class CompanyService {
  ...
  private async validations(company: Company): Promise<boolean> {
    const companyExists = await this.companyRepository.findOne({
      where: [{ email: company.email.toLowerCase() }, { cnpj: company.cnpj }],
    });
    if (companyExists) {
      return true;
    }
  ...
  public async findNearbyCompany(companyId: string, radius: number) {
    ...
    const query = this.companyRepository
      ...
      .where(
        `6371 * acos(cos(radians(:companyLatitude)) * cos(radians(address.latitude)) * cos(radians(address.longitude) - radians(:companyLongitude)) + sin(radians(:companyLatitude)) * sin(radians(address.latitude))) < :radius`,
        {
          companyLatitude,
          companyLongitude,
          radius,
