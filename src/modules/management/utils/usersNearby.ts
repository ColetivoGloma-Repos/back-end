...
export class UserNearby {
  ...
  public async usersNearby(management: Management, radius: number) {
    ...
    const query = this.userRepository
      ...
      .where(
        `6371 * acos(cos(radians(:managementLatitude)) * cos(radians(address.latitude)) * cos(radians(address.longitude) - radians(:managementLongitude)) + sin(radians(:managementLatitude)) * sin(radians(address.latitude))) < :radius`,
        {
          managementLatitude,
          managementLongitude,
          radius,
