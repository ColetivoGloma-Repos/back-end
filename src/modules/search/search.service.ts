...
export class SearchService {
  ...
  async findShelter(query: RequestShelterDTO): Promise<Shelter[]> {
    ...
    if (neighborhood) {
      queryBuilder.andWhere('address.bairro ILIKE :neighborhood', {
        neighborhood: `%${neighborhood}%`,
      });
    }
    if (street) {
      queryBuilder.andWhere('address.logradouro ILIKE :street', {
        street: `%${street}%`,
      });
    }
    if (city) {
      queryBuilder.andWhere('address.municipio ILIKE :city', {
        city: `%${city}%`,
      });
    }
    if (state) {
      queryBuilder.andWhere('address.estado ILIKE :state', {
        state: `%${state}%`,
      });
    }
  ...
  async findNeedVolunteer(query: RequestNeedsDTO): Promise<NeedVolunteers[]> {
    ...
    const queryBuilder = this.needVolunteerRepository
    ...
    if (shelter) {
      queryBuilder.andWhere('shelter.name ILIKE :shelter', {
        shelter: `%${shelter}%`,
      });
    }
    if (sortBy) {
    ...
    try {
      ...
      logger.error(error);
      throw new InternalServerErrorException(
        'Erro ao buscar necessidades de voluntariado',
      );
    }
  }
  ...
  async findNeedItem(query: RequestNeedsDTO): Promise<NeedItem[]> {
    ...
    const queryBuilder = this.needItemRepository
      .createQueryBuilder('n')
      .leftJoinAndSelect('n.shelter', 'shelter')
      .leftJoinAndSelect('n.coordinator', 'coordinator')
      .select([
